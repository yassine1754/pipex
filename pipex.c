...
/*   Updated: 2025/02/18 15:39:20 by yadib            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	forking(t_pairs *s_data, char **arg, int *pip)
{
	int	pid;
	...
	if (pid == 0)
		input_area_files(s_data, arg[2], pip);
	pid = fork();
	if (pid < 0)
		smart_exit(FORK_FAIL, NULL, s_data);
	else if (pid == 0)
		output_area_files(s_data, arg[3], pip);
	else if (pid > 0)
	{
		close(pip[0]);
		close(pip[1]);
...
int	main(int ac, char *argv[], char **env)
{
	...
	{
		ft_putstr_fd("Error\n", 2);
		ft_putstr_fd("Enter ./pipex infile cmd1 cmd2 outfile\n", 2);
		exit(1);
	}
	...
	forking(&s_data, argv, s_data.pip);
}
